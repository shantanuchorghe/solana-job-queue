...
const CreateJobForm_1 = __importDefault(require("./CreateJobForm"));
const solana_1 = require("./solana");
const useSolanaWallet_1 = require("./useSolanaWallet");
const DEFAULT_QUEUE = (_a = import.meta.env.VITE_DECQUEUE_DEFAULT_QUEUE) !== null && _a !== void 0 ? _a : "";
...
const PRIORITY_LABELS = { 0: "LOW", 1: "NORMAL", 2: "HIGH" };
const PRIORITY_COLORS = { 0: "#6b7280", 1: "#3b82f6", 2: "#f59e0b" };
const STATUS_FILTERS = ["all", "pending", "processing", "completed", "failed", "cancelled"];
function truncate(value, head = 8, tail = 4) {
...
function statusCount(snapshot, filter) {
    if (!snapshot) {
        return 0;
    }
    if (filter === "all") {
...
function JobRow({ job, onClick, selected, onCancel, isCancelling, }) {
    return (<div onClick={onClick} style={{
      ...
      <span style={{ textAlign: "right" }}>
        {(job.status === "pending" || job.status === "processing") ? (<button onClick={(e) => {
                e.stopPropagation();
                onCancel === null || onCancel === void 0 ? void 0 : onCancel(job);
            }} disabled={isCancelling} style={{
...
function FieldRow({ label, value }) {
    return (<div style={{ borderBottom: "1px solid #1a1a1a", padding: "10px 0", display: "flex", gap: 16 }}>
...
function JobDetail({ job, onClose }) {
    ...
    return (<div style={{
      ...
      <FieldRow label="WORKER" value={(_a = job.worker) !== null && _a !== void 0 ? _a : "-"}/>
      ...
      {job.result ? (<div style={{ marginTop: 16 }}>
      ...
      {job.errorMessage ? (<div style={{ marginTop: 16 }}>
...
function EmptyState({ queueInput, onUseDefault, }) {
    return (<div style={{
            margin: "48px auto",
            maxWidth: 820,
            border: "1px solid #1e1e1e",
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 1, background: "#141414" }}>
        <StatCard label="Program" value={truncate((0, solana_1.getProgramId)(), 8, 6)} accent="#3b82f6" sublabel="same program id on localnet/devnet"/>
      ...
      {DEFAULT_QUEUE ? (<button onClick={onUseDefault} style={{
                marginTop: 20,
                background: "#0d2040",
...
function App() {
    var _a, _b, _c;
    const [cluster, setCluster] = (0, react_1.useState)("devnet");
    const [queueInput, setQueueInput] = (0, react_1.useState)(DEFAULT_QUEUE);
    const [activeQueue, setActiveQueue] = (0, react_1.useState)(DEFAULT_QUEUE);
    const [filter, setFilter] = (0, react_1.useState)("all");
    const [snapshot, setSnapshot] = (0, react_1.useState)(null);
    const [events, setEvents] = (0, react_1.useState)([]);
    const [selectedJobKey, setSelectedJobKey] = (0, react_1.useState)(null);
    const [loading, setLoading] = (0, react_1.useState)(false);
    const [error, setError] = (0, react_1.useState)(null);
    const [cancellingJobIds, setCancellingJobIds] = (0, react_1.useState)(new Set());
    const wallet = (0, useSolanaWallet_1.useSolanaWallet)();
    const refreshQueue = (0, react_1.useCallback)((...args_1) => __awaiter(this, [...args_1], void 0, function* (quiet = false) {
        if (!activeQueue) {
            return;
        }
        try {
            ...
            setError(null);
            const nextSnapshot = yield (0, solana_1.fetchQueueSnapshot)(cluster, activeQueue);
    ...
    (0, react_1.useEffect)(() => {
        ...
        void (0, solana_1.subscribeToQueueEvents)(cluster, activeQueue, (event) => {
            ...
            setEvents((previous) => [
                {
                    ...
                    text: `EVENT STREAM UNAVAILABLE: ${eventError.message}`,
                    timestamp: new Date(),
                    kind: "system",
                },
                ...previous,
    ...
    const selectedJob = (0, react_1.useMemo)(() => { var _a; return (_a = snapshot === null || snapshot === void 0 ? void 0 : snapshot.jobs.find((job) => job.publicKey === selectedJobKey)) !== null && _a !== void 0 ? _a : null; }, [selectedJobKey, snapshot]);
    const successRate = snapshot && snapshot.queue.totalJobs > 0
        ? ((snapshot.queue.completedJobs / snapshot.queue.totalJobs) * 100).toFixed(1)
        : "0.0";
    function handleLoadQueue(event) {
        event.preventDefault();
        setSelectedJobKey(null);
        setActiveQueue(queueInput.trim());
    }
    const handleJobCreated = (0, react_1.useCallback)((result) => __awaiter(this, void 0, void 0, function* () {
            ...
            return [
                {
                    ...
                    text: `JOB #${result.jobId} ENQUEUED FROM ${(_a = wallet.shortAddress) !== null && _a !== void 0 ? _a : "WALLET"}`,
                    timestamp: new Date(),
                    kind: "system",
                },
                ...previous,
        ...
        yield refreshQueue(true);
    }), [refreshQueue, wallet.shortAddress]);
    const handleCancelJob = (0, react_1.useCallback)((job) => __awaiter(this, void 0, void 0, function* () {
        if (!wallet.publicKey) {
            return alert("Wallet not connected");
        }
        setCancellingJobIds((prev) => new Set(prev).add(job.publicKey));
        try {
            yield (0, solana_1.cancelJobFromWallet)({
                cluster,
    ...
    return (<div style={{
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 16, flexWrap: "wrap" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <img src="/decqueue-logo.png" alt="DecQueue" style={{
            width: 28,
            height: 28,
            borderRadius: 6,
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
          {wallet.availableWallets.length > 1 ? (<select value={(_a = wallet.selectedWalletId) !== null && _a !== void 0 ? _a : ""} onChange={(event) => void wallet.selectWallet(event.target.value)} style={Object.assign(Object.assign({}, inputStyle), { width: 140, padding: "9px 12px" })}>
              {wallet.availableWallets.map((walletOption) => (<option key={walletOption.id} value={walletOption.id}>
                  {walletOption.label}
                </option>))}
            </select>) : wallet.selectedWalletLabel ? (<div style={{ color: "#555", fontSize: 10 }}>
              APP: <span style={{ color: "#777" }}>{wallet.selectedWalletLabel}</span>
            </div>) : null}
          <div style={{ color: wallet.connected ? "#22c55e" : "#555", fontSize: 10 }}>
            WALLET: <span style={{ color: wallet.connected ? "#86efac" : "#777" }}>{(_b = wallet.shortAddress) !== null && _b !== void 0 ? _b : "DISCONNECTED"}</span>
          </div>
          {wallet.connected ? (<>
              <button onClick={() => void wallet.switchWallet()} disabled={wallet.connecting || !wallet.available} style={Object.assign(Object.assign({}, buttonStyle), { color: wallet.available ? "#f59e0b" : "#444", borderColor: wallet.available ? "#f59e0b" : "#222" })}>
                {wallet.connecting ? "SWITCHING" : "SWITCH ACCOUNT"}
              </button>
              <button onClick={() => void wallet.disconnect()} disabled={wallet.connecting} style={Object.assign(Object.assign({}, buttonStyle), { color: "#86efac", borderColor: "#22c55e" })}>
                DISCONNECT
              </button>
            </>) : (<button onClick={() => void wallet.connect()} disabled={wallet.connecting || !wallet.available} style={Object.assign(Object.assign({}, buttonStyle), { color: wallet.available ? "#3b82f6" : "#444", borderColor: wallet.available ? "#3b82f6" : "#222" })}>
              {wallet.connecting ? "CONNECTING" : `CONNECT ${(_c = wallet.selectedWalletLabel) !== null && _c !== void 0 ? _c : "WALLET"}`}
            </button>)}
          <div style={{ color: "#333", fontSize: 10 }}>
            PROGRAM: <span style={{ color: "#555" }}>{truncate((0, solana_1.getProgramId)(), 8, 8)}</span>
          </div>
          <button onClick={() => void refreshQueue()} disabled={!activeQueue || loading} style={Object.assign(Object.assign({}, buttonStyle), { color: activeQueue ? "#3b82f6" : "#444", borderColor: activeQueue ? "#3b82f6" : "#222" })}>
      ...
      <div style={{ padding: "18px 24px", borderBottom: "1px solid #141414", background: "#070707" }}>
        <form onSubmit={handleLoadQueue} style={{ display: "grid", gridTemplateColumns: "120px 1fr 140px 120px", gap: 12 }}>
          <select value={cluster} onChange={(event) => setCluster(event.target.value)} style={inputStyle}>
            <option value="devnet">devnet</option>
            <option value="localnet">localnet</option>
          </select>
          <input value={queueInput} onChange={(event) => setQueueInput(event.target.value)} style={inputStyle} placeholder="Paste queue PDA"/>
          <button type="submit" style={Object.assign(Object.assign({}, buttonStyle), { background: "#0d2040", borderColor: "#3b82f6", color: "#3b82f6" })}>
            LOAD QUEUE
          </button>
        ...
        {error ? (<div style={{ marginTop: 12, color: "#fca5a5", background: "#1f0e0e", border: "1px solid #3a1e1e", padding: "10px 12px", fontSize: 11 }}>
            {error}
          </div>) : null}

        {wallet.error ? (<div style={{ marginTop: 12, color: "#fcd34d", background: "#1f1a0e", border: "1px solid #4b3a12", padding: "10px 12px", fontSize: 11 }}>
            {wallet.error}
          </div>) : null}
      </div>

      {!snapshot ? (<EmptyState queueInput={queueInput} onUseDefault={() => {
                setQueueInput(DEFAULT_QUEUE);
                setActiveQueue(DEFAULT_QUEUE);
          ...
          <div style={{ borderRight: "1px solid #141414", background: "#080808", padding: "20px 0" }}>
            ...
            <div style={{ padding: "16px 20px", borderBottom: "1px solid #141414" }}>
              ...
              {STATUS_FILTERS.map((status) => {
                ...
                return (<button key={status} onClick={() => setFilter(status)} style={{
                        display: "flex",
                        justifyContent: "space-between",
                        alignItems: "center",
                        width: "100%",
                        ...
                        border: "none",
                        borderLeft: active ? `2px solid ${(_a = colors === null || colors === void 0 ? void 0 : colors.accent) !== null && _a !== void 0 ? _a : "#3b82f6"}` : "2px solid transparent",
                        color: active ? ((_b = colors === null || colors === void 0 ? void 0 : colors.text) !== null && _b !== void 0 ? _b : "#93c5fd") : "#555",
                        cursor: "pointer",
                        padding: "7px 12px",
                        fontSize: 10,
                        letterSpacing: 1,
                    ...
                    <span style={{ color: active ? ((_c = colors === null || colors === void 0 ? void 0 : colors.accent) !== null && _c !== void 0 ? _c : "#3b82f6") : "#333", fontSize: 10 }}>{count}</span>
                  </button>);
            })}
            </div>
          ...
          <div style={{ display: "flex", flexDirection: "column" }}>
            ...
            <CreateJobForm_1.default cluster={cluster} queue={snapshot.queue} wallet={wallet} onJobCreated={handleJobCreated}/>
            ...
            <div style={{ display: "grid", gridTemplateColumns: selectedJob ? "1fr 380px" : "1fr", flex: 1, overflow: "hidden" }}>
              <div style={{ overflow: "auto" }}>
                ...
                background: "#080808",
                  ...
                  {["ID", "JOB TYPE", "STATUS", "PRIORITY", "RETRIES", "AGE", ""].map((heading, idx) => (<span key={idx} style={{ color: "#333", fontSize: 9, letterSpacing: 2 }}>
                      {heading}
                    </span>))}
                </div>

                {filteredJobs.length === 0 ? (<div style={{ color: "#333", fontSize: 11, textAlign: "center", padding: 60 }}>
                    NO JOBS MATCH THIS FILTER
                  </div>) : (filteredJobs.map((job) => (<JobRow key={job.publicKey} job={job} onClick={() => setSelectedJobKey((current) => (current === job.publicKey ? null : job.publicKey))} selected={selectedJobKey === job.publicKey} onCancel={handleCancelJob} isCancelling={cancellingJobIds.has(job.publicKey)}/>)))}
              </div>

              {selectedJob ? (<div style={{ borderLeft: "1px solid #1a1a1a", overflow: "auto" }}>
            ...
            <div style={{
              ...
              <div style={{ display: "flex", gap: 16, color: "#333", fontSize: 9, flexWrap: "wrap" }}>
                <a href={`https://explorer.solana.com/address/${snapshot.queue.publicKey}?cluster=${cluster}`} target="_blank" rel="noopener noreferrer" style={{ color: "#3b82f650", textDecoration: "none" }}>
